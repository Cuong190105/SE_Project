...
class _MeaningListState extends State<MeaningList> {
  ...
  Future<List<Map<String, dynamic>>> parseWordDetails(
      String htmlContent, String htmlContentEnglish) async {
    ...
    final documentEnglish = html_parser
        .parse(extractUlAfterPronunciationHeading(htmlContentEnglish));
    ...
    for (var section in filteredSections) {
      ...
      for (int i = 0; i < sectionElements.length; i++) {
        ...
        final partOfSpeechElement =
            sectionElements[i].querySelector('.mw-heading h3');
        String partOfSpeech =
            partOfSpeechElement?.text.trim() ?? 'không rõ từ loại';
        ...
        if (!phoneticElements.isEmpty) {
          phoneticUK = phonetic(
              extractUlAfterPronunciationHeading(htmlContentEnglish),
              partOfSpeech,
              phoneticElements.first.text.trim(),
              'UK');
          phoneticUS = phonetic(
              extractUlAfterPronunciationHeading(htmlContentEnglish),
              partOfSpeech,
              phoneticElements.last.text.trim(),
              'US');
        } else {
          phoneticUK = phonetic(
              extractUlAfterPronunciationHeading(htmlContentEnglish),
  ...
  String phonetic(String html, String tuLoai, String oldp, String nation) {
    ...
    for (var li in ul.children.where((e) => e.localName == 'li')) {
      final spans =
          li.children.where((child) => child.localName == 'span').toList();
  ...
  Future<List<String>> audioList(String html, String tuLoai) async {
    ...
    String type = chuyenTuLoaiSangTiengAnh(tuLoai);
    final bodyChildren = document.body?.children;
    if (bodyChildren == null) print(1);
    if (bodyChildren == null || bodyChildren.isEmpty) print(2);
    ...
    for (var li in ul.children.where((e) => e.localName == 'li')) {
      ...
      if (spanText.contains(type)) {
        ...
        final audioSources = li.querySelectorAll('source');
        if (audioSources.isNotEmpty) {
          String temp = await validateAudioUrl(
              audioSources.first.attributes['src'] ?? '');
    ...
    if (audioSources.isNotEmpty) {
      String temp =
          await validateAudioUrl(audioSources.first.attributes['src'] ?? '');
      if (temp.isNotEmpty) {
        listAudioUrl[0] = temp;
        listAudioUrl[1] = temp;
      }
            ...
            child: Column(
              ...
              children: [
                ...
                GestureDetector(
                  ...
                  child: Tooltip(
                    message:
                        'Đi đến trang thêm từ', // Nội dung tooltip khi di chuột
                    child: Icon(
                  ...
                  children: [
                    ...
                    Column(
                      children: [
                        GestureDetector(
                          onTap: () async {
                            //   if (!'${wordType['audio']}'.isEmpty)
                            //     _playPreloadedAudio(wordType['audio'][0]);
                          }, // US audio
                          child: Row(
                            children: [
                              Icon(Icons.volume_up,
                                  size: 20,
                                  color: '${wordType['audio']}'.isEmpty
                                      ? Colors.grey
                                      : Colors.black),
                              SizedBox(width: 5),
                              Text(
                        ...
                        GestureDetector(
                          onTap: () async {
                            // if (!'${wordType['audio']}'.isEmpty)
                            //   _playPreloadedAudio(wordType['audio'][1]);
                          },
                          child: Row(
                            children: [
                              Icon(Icons.volume_up,
