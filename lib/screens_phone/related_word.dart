...
class _RelatedWordsListState extends State<RelatedWordsList> {
  ...
  Future<void> fetchRelatedWords(String word) async {
    ...
    try {
      ...
      if (responses.every((res) => res.statusCode == 200)) {
        ...
        final familyWords = allFamilyCandidates
            .where((w) {
              final wI =
                  w.endsWith('y') ? w.substring(0, w.length - 1) + 'i' : w;
        ...
        setState(() {
          relatedWords = {
            "synonyms": synonymData
                    ...
                    .toList()
                    .isNotEmpty
                ? synonymData.map((item) => item['word'] as String).toList()
                : ['Không có từ đồng nghĩa'],
            "antonyms": antonymData
                    .map((item) => item['word'] as String)
                    .toList()
                    .isNotEmpty
                ? antonymData.map((item) => item['word'] as String).toList()
                : ['Không có từ trái nghĩa'],
            "wordFamily":
                familyWords.isNotEmpty ? familyWords : ['Không có họ từ vựng'],
            "phrases": [' '],
          };
          isLoading = false;
        });
    ...
    setState(() {
      relatedWords['phrases'] =
          idioms.isNotEmpty ? idioms : ['Không có cụm từ'];
    });
  }
